...
CONTAINS
!
subroutine SYMB_DRIVER(Nf, Nd, Na, debug, qSymbolic, Nmc, frm)
  implicit none
...
!
  IF (frmt == 'csv') THEN 
      Call write_csvs(Nframes, Natoms, Ndim, XYZS) 
  ELSEIF (frmt == 'xyz') THEN 
      Call write_xyzs(Nframes, Natoms, Ndim, XYZS)  
  ELSE
      write(*,*) frmt 
	  STOP 'Unknown format of symbolic time series'
  ENDIF    
!
