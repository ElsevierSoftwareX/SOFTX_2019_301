...
CONTAINS
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine SMI_DRIVER(frm, Nf, Nd, Na, qMIShuffle, debug, Nshuffles, r0, statP)
  USE TEMPI_CLASS
  implicit none
...
!
  call read_embdparam(Natoms, Ndim, Mopt, Tau)
!
! -- Initialize MPI
  call MPI_start()
...
! -- Timing Analysis  
  ...
  ENDIF
!
  IF (myID == Master) CALL write_mi(Natoms, MIS, MI, qMIShuffle) 
!
  CALL DEAllocate_MI()
!
  call MPI_finish()
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine MI_DRIVER(frm, Nf, Nd, Na, qMIShuffle, debug, Nshuffles, r0, statP)
  USE TEMPI_CLASS
  implicit none
  integer, intent(in)        :: qMIShuffle
  integer, intent(in)        :: Na, Nf
