...
CONTAINS
!
subroutine EMBD_DRIVER(Nf, Nd, Na, M1, M2, Tau1, tau2, db, frm )
  use TEMPI_CLASS
  implicit none
...
!
  ...
  ELSEIF (frmt == 'csv') THEN
      Call Read_csv(Nframes, Natoms, Ndim, X)  
  ELSE 
      write(*,*) frmt 
	  STOP "Undefined output format"
  ENDIF 
!
  call MPI_start()
  call MPI_Broadcast()
...
!
  ...
  Call print_embdparam(Natoms, Ndim, Mopt, Tau)
  write(*,*) "... Done!"
!
...
IF (myID == master) THEN
    nunit1 = new_unit()
    open(unit=nunit1, file='tau.csv', status='unknown', action='write')
    nunit2 = new_unit()
    open(unit=nunit2, file='mopt.csv', status='unknown', action='write')
ENDIF
!
...
!
IF ( myID == master ) THEN
     write(nunit2,*) ("m-value", ",", "Optimized m",",", I=1, Ndof)
     DO m = mopt_min, mopt_max
        write(nunit2,*) (m, ",", mopt(i), ",", I=1, Ndof)
    ENDDO
    close(nunit2)
ENDIF
!
IF ( myID == master ) THEN
     write(nunit1,*) ("Tau", ",", "Optimized Tau",",", I=1, Ndof)
     DO m = topt_min, topt_max
        write(nunit1,*) (m, ",", Tau(i), ",", I=1, Ndof)
    ENDDO
    close(nunit1)
ENDIF
!
...
!
IF ( myID == master ) THEN
     write(nunit2,*) ("m-value", ",", "Count-fnn", ",", "Optimized m", ",", I=1, Ndof)
     im = 0
     DO m = mopt_min, mopt_max
        im = im + 1
        write(nunit2,*) (m, ",", gcount_fnn(im), ",", Gmopt(i), ",", I=1, Ndof)
    ENDDO
	close(nunit2)
ENDIF
!
IF ( myID == master ) THEN
     write(nunit1,*) ("Tau", ",", "Mutual Information(Tau)", ",", "Optimized Tau", ",", I=1, Ndof)
     im = 0
     DO m = topt_min, topt_max
        im = im + 1
        write(nunit1,*) (m, ",", gMi(im), ",", gTau(i), ",", I=1, Ndof)
    ENDDO
	close(nunit1)
ENDIF
!
