...
CONTAINS
!
subroutine Setup_DRIVER(frm, Nf, Nd, Na, Off,                            &
                       iStart, iSkip, iStop,                        &
                       db, benchmark,                               &
...
!
  ...
  write(*,*) "... allocated"
  Allocate( X(Natoms*Ndim, Nframes), stat=ierr )
...
!
  IF (frmt == 'csv') THEN 
      call write_csv(Nframes, Natoms, Ndim, XYZ)
      IF ( (benchmark == 4) .or. (benchmark == 5) ) THEN
            call write_csvs(Nframes, Natoms, Ndim, XYZs)      
      ENDIF
  ELSE IF (frmt == 'xyz') THEN 
      call write_xyz(Nframes, Natoms, Ndim, XYZ)
      IF ( (benchmark == 4) .or. (benchmark == 5) ) THEN
