...
implicit none
!
      integer, save :: nframes, ndim, natoms, ndof 
	  ...
	  character(len=3), save :: frmt 

!
CONTAINS
!
subroutine STE1_DRIVER(frm, Nf, Nd, Na, qteShuffle, qteNorm, debug, &
                      Nshuffles, r0, statP)
  use tempi_class
  implicit none
...
!
  ...
  IF (frmt == 'csv') THEN 
      call read_csvs(Nframes, Natoms, Ndim, XS)
  ELSEIF (frmt == 'xyz') THEN 
      call read_xyzs(Nframes, Natoms, Ndim, XS)  
  ELSE
      write(*,*) frmt 
	  STOP 'Unknown format of time series'
  ENDIF
  J = 0
  DO i = 1, Natoms
     DO d = 1, Ndim
...
!
  call read_embdparam(Natoms, Ndim, Mopt, Topt)
  Mopt = 1
!
! -- Initialize MPI
  call MPI_start()
...
! -- Timing Analysis  
  ...
  ENDIF
!
  IF (myID == Master) call WRITE_TE(Natoms, TES, TE, qTeShuffle, Ndim, qTeShuffle, 1)
! 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine STE2_DRIVER(frm, Nf, Nd, Na, qteShuffle, qteNorm, debug, &
                      Nshuffles, r0, statP)
  use tempi_class
  implicit none
  integer, intent(in)        :: qTeNorm
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine STE3_DRIVER(frm, Nf, Nd, Na, qteShuffle, qteNorm, debug, &
                      Nshuffles, r0, statP)
  use tempi_class
  implicit none
  integer, intent(in)        :: qTeNorm
...
! -- Timing Analysis  
  ...
  ENDIF
!
  IF (myID == Master) call WRITE_TE(Natoms, TES, TE, qTeShuffle, Ndim, qTeShuffle, 3)
! 
  CALL DEALLOCATE_TransferEntropy()
!
  call MPI_finish()
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine TE_DRIVER(frm, Nf, Nd, Na, qteShuffle, qteNorm, debug, &
                      Nshuffles, r0, statP)
  use tempi_class
  implicit none
  integer, intent(in)        :: qTeNorm
...
!
  ...
  IF (frmt == 'csv') THEN 
      call read_csv(Nframes, Natoms, Ndim, X)
  ELSEIF (frmt == 'xyz') THEN 
      call read_xyz(Nframes, Natoms, Ndim, X)  
  ELSE
      write(*,*) frmt 
	  STOP 'Unknown format of time series'
  ENDIF
...
! -- Timing Analysis  
  ...
  ENDIF
!
  IF (myID == Master) call WRITE_TE(Natoms, TES, TE, qTeShuffle, Ndim, qTeShuffle, 4)
! 
  CALL DEALLOCATE_TransferEntropy()
!
  call MPI_finish()
...
!
ELSE
     Stop "Number of Pairs must be Multiple of Number of Processors"
ENDIF
!
